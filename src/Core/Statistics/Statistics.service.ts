...
import { SAHMANAHATUM_SEQUELIZE, STATISTICS_SEQUELIZE, WP_SEQUELIZE } from 'src/Core/Statistics/statistics.tokens';
import { StatisticsPeriodOption, StatisticsRow } from 'src/Core/Statistics/interfaces/statistics.interfaces';
...
export class StatisticsService {
  ...
  async uploadBorderCrossFile(_body: Record<string, unknown>) {
  ...
  async getSimpleWPStatistics(body: WpSimpleStatsRequestDto) {
    ...
    switch (wp_type) {
      ...
      case 'work_permit':
        query =
          report_type === 1
            ? formatWpApplicationsQuery({ year, month, period, claim_type, report_type, decType })
            : formatWpDecisionsQuery({ year, month, period, claim_type, report_type, decType });
        break;
      case 'volunteer':
        query =
          report_type === 1
  ...
  async exportExcel(_body: ExportStatsRequestDto) {
    throw new NotImplementedException('File export logic is not migrated.');
  }

  async exportPdf(_body: ExportStatsRequestDto) {
    throw new NotImplementedException('File export logic is not migrated.');
  }
