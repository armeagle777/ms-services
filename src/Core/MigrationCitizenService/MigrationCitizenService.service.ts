...
export class MigrationCitizenService {
   ...
   async searchPersons(body: McsSearchPersonsDto): Promise<McsPersonRecord[]> {
      const { addressType, firstName, lastName, patronomicName, building, street, community } =
         body || {};
   ...
   private async getPersonsByAddress(filters: McsSearchPersonsDto): Promise<McsPersonRecord[]> {
      ...
      const body = {
         request: { number: '1101', department: '999' },
         regist_in_addr:
            filters.registrationType === 'EVER' ? 'EVER_REGISTERED' : 'CURRENTLY_REGISTERED',
         region,
         community,
         residence,
         street,
   ...
   private async searchPersonByBirthAddress(
