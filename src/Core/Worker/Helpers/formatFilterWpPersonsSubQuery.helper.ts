...
export const formatFilterWpPersonsSubQuery = (filters: Filters) => {
  ...
  if (fisrt_name_lat) {
    ...
    replacements.fisrt_name_lat = `%${fisrt_name_lat}%`;
  }

  if (last_name_lat) {
    baseQuery += ` AND last_name_en LIKE :last_name_lat`;
    replacements.last_name_lat = `%${last_name_lat}%`;
  }

  if (fisrt_name_arm) {
    baseQuery += ` AND first_name_am LIKE :fisrt_name_arm`;
    replacements.fisrt_name_arm = `%${fisrt_name_arm}%`;
  }

  if (last_name_arm) {
    baseQuery += ` AND last_name_am LIKE :last_name_arm`;
    replacements.last_name_arm = `%${last_name_arm}%`;
  }

  if (psn) {
    baseQuery += ` AND ALL_PERSON.ssn = :psn`;
    replacements.psn = psn;
  }

  if (document_number) {
    baseQuery += ` AND passport_number = :document_number`;
    replacements.document_number = document_number;
  }
  ...
  if (select_country?.value && select_country.value != 0) {
    baseQuery += ` AND citizenship_id = :select_country`;
